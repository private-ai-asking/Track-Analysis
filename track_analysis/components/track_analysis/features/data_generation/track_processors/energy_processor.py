...
class EnergyTrackProcessor(ITrackProcessorStrategy):
    def __init__(self, logger: HoornLogger):
        super().__init__(logger, "Energy", is_child=True)
    ...
    def process_track(self, track: AudioInfo) -> AudioInfo:
        ...
        for metadata_item_original in track.metadata:
            if metadata_item_original.header == Header.Energy_Level:
                continue

            updated_metadata.append(metadata_item_original.model_copy(deep=True))


        updated_metadata.append(AudioMetadataItem(header=Header.Energy_Level, description="The energy level of the track.", value=file.get('energylevel', ["Unknown"])[0]))
