...
class CacheBuilderPipeline(AbPipeline):
    def __init__(self, logger: HoornLogger, scrobble_utils: ScrobbleUtility, embedder: SentenceTransformer,
                 parameters: ScrobbleCacheAlgorithmParameters, cache_helper: ScrobbleCacheHelper, test_mode: bool, form_gold_standard: bool = False):
    ...
    def build_pipeline(self):
        ...
        self._add_step(status_report)
        self._add_step(NearestNeighborSearch(self._logger, self._scrobble_utils, self._embedder, self._parameters, test_mode=self._test_mode))
