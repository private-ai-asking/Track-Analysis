...
class NormalizedPitchClassesCleaner:
    ...
    def clean(self, normalized: np.ndarray, raw_audio_samples: np.ndarray, n_fft: int, hop_length: int, sample_rate: int) -> Tuple[np.ndarray, np.ndarray]:
        envelope_db = _compute_rms_envelope(raw_audio_samples, n_fft, hop_length)

        cleaned_pass1 = _first_clean_pass(envelope_db, normalized)
        ...
        cleaned_pass2 = _second_clean_pass(cleaned_pass1)
        perc_changed_pass2 = _get_perc_diff(cleaned_pass1, cleaned_pass2)
        ...
        cleaned_pass3 = _third_clean_pass(cleaned_pass2)
        perc_changed_pass3 = _get_perc_diff(cleaned_pass2, cleaned_pass3)
        ...
        cleaned_pass4 = _fourth_clean_pass(cleaned_pass3)
        perc_changed_pass4 = _get_perc_diff(cleaned_pass3, cleaned_pass4)
        ...
        cleaned_pass5_binary = _fifth_clean_pass(cleaned_pass4, sample_rate, hop_length)
        perc_changed_pass5 = _get_perc_diff(cleaned_pass4, cleaned_pass5_binary)
        ...
        cleaned_pass5_chroma = _convert_back(cleaned_pass5_binary, normalized)
